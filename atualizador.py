...
def obter_detalhes_camara(id_proposicao):
    """Faz chamadas secundárias na Câmara para pegar Autor e Comissão"""
    ...
    # 2. Busca Última Tramitação (Comissão)
    try:
    ...
    return autor_formatado, comissao_atual
    ...
    if response.status_code == 200:
        ...
        for p in proposicoes:
            ...
            if 'planejamento' in ementa or 'mpo' in ementa or 'orçamento' in ementa:
                ...
                autor, comissao = obter_detalhes_camara(id_prop)
                
                # Extrai o mês/ano para a linha do tempo (ex: "05/2026")
                data_str = p.get('dataApresentacao', f'{ANO_BUSCA}-01-01T00:00').split('T')[0]
                data_obj = datetime.strptime(data_str, '%Y-%m-%d')
                mes_ano = data_obj.strftime('%b/%y').capitalize()
